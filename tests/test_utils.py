...
def test_get_list_card(df_transactions) -> None:
    assert get_list_cards(date_time="2005-11-18 10:45:15", df_filter=df_transactions) == [
        {
            "cashback": 1.66,
            "last_digits": "7856",
            "total_spent": 165.89,
...
def test_get_currency_rates(mock_get) -> None:
    mock_get.return_value.json.return_value = {"result": 80.85}
    assert get_currency_rates() == [{"currency": "USD", "rate": 80.85}, {"currency": "EUR", "rate": 80.85}]
...
def test_get_stock_prices(mock_get):
    ...
    assert get_stock_prices() == [
        {"stock": "AAPL", "price": 2.05},
        {"stock": "AMZN", "price": 2.05},
        {"stock": "GOOGL", "price": 2.05},
        {
            "stock": "MSFT",
            "price": 2.05,
        },
        {"stock": "TSLA", "price": 2.05},
    ]
